  ...
  #define WIFI_RECON_INTERVAL       0         // default 0s between reconnecting WiFi
    ...
    void run()
    {
      ...
      else if (configuration_mode)
      {
        if(wps_active){
          wps_active = false;
          ESP_WML_LOGWARN5(F("WPS Connected: "), WiFi.localIP()," ", WiFi.SSID(), " ", WiFi.macAddress());
          ESP_WML_LOGWARN1(F("WPS SSID: "), WiFi.SSID());
          ESP_WML_LOGWARN1(F("WPS PW: "), WiFi.psk());

            if (strlen(WiFi.SSID().c_str()) < sizeof(ESP_WM_LITE_config.WiFi_Creds[0].wifi_ssid) - 1)
              strcpy(ESP_WM_LITE_config.WiFi_Creds[0].wifi_ssid, WiFi.SSID().c_str());
            else
              strncpy(ESP_WM_LITE_config.WiFi_Creds[0].wifi_ssid, WiFi.SSID().c_str(), sizeof(ESP_WM_LITE_config.WiFi_Creds[0].wifi_ssid) - 1);

            if (strlen(WiFi.psk().c_str()) < sizeof(ESP_WM_LITE_config.WiFi_Creds[0].wifi_pw) - 1)
              strcpy(ESP_WM_LITE_config.WiFi_Creds[0].wifi_pw, WiFi.psk().c_str());
            else
              strncpy(ESP_WM_LITE_config.WiFi_Creds[0].wifi_pw, WiFi.psk().c_str(), sizeof(ESP_WM_LITE_config.WiFi_Creds[0].wifi_pw) - 1);

            strcpy(ESP_WM_LITE_config.header, ESP_WM_LITE_BOARD_TYPE);

            if (isWiFiConfigValid()){
              ESP_WML_LOGWARN(F("!! WiFiConfig Valid"));
            } else {
              ESP_WML_LOGWARN(F("!! WiFiConfig NOT Valid"));
            }
...
#if USE_WIFI_WPS
bool wps_active = false;
    void startWPSConfigurationMode()
    {
      ESP_WML_LOGWARN(F("WPS active"));
      WiFi.beginWPSConfig();
      wps_active = true;
    }
#endif
